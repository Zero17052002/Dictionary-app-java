...
/**
 * Utility class for interacting with the Microsoft Translator Text API for
 * translation.
 */
public class TranslateAPI {
	 ...
	 * @return A TranslationResponse object containing the translated text and other
	 *         details.
	 */
	public TranslationResponse translate(String text) {
		...
		Request request = new Request.Builder()
				.url("https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&from=en&to=vi").post(body)
				.addHeader("Ocp-Apim-Subscription-Key", API_KEY).addHeader("Ocp-Apim-Subscription-Region", API_LOCATION)
	...
	/**
	 * Parses the JSON response from the translation API into a TranslationResponse
	 * object.
	 *
	...
	private TranslationResponse parseTranslationResponse(String jsonText) {
		...
		String detectedSourceLanguage = jsonObject.has("detectedLanguage")
				? jsonObject.get("detectedLanguage").getAsString()
				: null;
	...
	public static void main(String[] args) {
		...
		TranslationResponse response = translator.translate(textToTranslate);

		if (!response.isError()) {
			List<String> translatedTexts = response.getTranslatedTexts();
			for (String translatedText : translatedTexts) {
				System.out.println("Translated Text: " + translatedText);
			}
		} else {
			System.err.println("Translation Error: " + response.getErrorMessage());
		}
	}
}
